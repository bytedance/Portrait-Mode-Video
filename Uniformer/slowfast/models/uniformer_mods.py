...
model_path = f"/mnt/bn/{os.environ['BYTENAS_VOLUME_NAME']}/pretrained_model"
# model_path = f"/mnt/bn/kinetics-pl-hl/kinetics-pl-hl/pretrained_model"
model_path = {
    ...
    'uniformer_small_pm_k400_16x4': os.path.join(model_path, 'uniformer_small_pm_k400_16x4.pth'),
...
def pm_conv_3xnxn(inp, oup, kernel_size=[3,3], stride=[3,3], groups=1):
    return nn.Conv3d(inp, oup, (3, kernel_size[0], kernel_size[1]), (2, stride[0], stride[1]), (1, 0, 0), groups=groups)
...
def pm_conv_1xnxn(inp, oup, kernel_size=[3,3], stride=[3,3], groups=1):
    return nn.Conv3d(inp, oup, (1, kernel_size[0], kernel_size[1]), (1, stride[0], stride[1]), (0, 0, 0), groups=groups)
...
def pm_conv_3xnxn_std(inp, oup, kernel_size=[3,3], stride=[3,3], groups=1):
    return nn.Conv3d(inp, oup, (3, kernel_size[0], kernel_size[1]), (1, stride[0], stride[1]), (1, 0, 0), groups=groups)
...
class SpeicalPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=768):
        super().__init__()
        conv_op = pm_conv_3xnxn
        if not isinstance(patch_size, list):
            assert not isinstance(stride, list)
            patch_size = to_2tuple(patch_size)
            conv_op = conv_3xnxn
        if not isinstance(stride, list):
            assert not isinstance(patch_size, list)
        ...
        self.proj = conv_op(in_chans, embed_dim, kernel_size=patch_size, stride=stride)
...
class PatchEmbed(nn.Module):
    """ Image to Patch Embedding
    """
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=768, std=False):
        super().__init__()
        if std:
            conv_op = pm_conv_3xnxn_std
        else:
            conv_op = pm_conv_1xnxn
        if not isinstance(patch_size, list):
            assert not isinstance(stride, list)
            patch_size = to_2tuple(patch_size)
            if std:
                conv_op = conv_3xnxn_std
            else:
                conv_op = conv_1xnxn
        if not isinstance(stride, list):
            assert not isinstance(patch_size, list)
        ...
        self.num_patches = num_patches
        self.norm = nn.LayerNorm(embed_dim)
        self.proj = conv_op(in_chans, embed_dim, kernel_size=patch_size[0], stride=patch_size[0])
...
class Uniformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        patch_size = cfg.UNIFORMER.PATCH_KERNEL
        stride = cfg.UNIFORMER.PATCH_KERNEL

        if isinstance(patch_size, list) and len(patch_size) == 0:
            patch_size = 4
        if isinstance(stride, list) and len(stride) == 0:
            stride = 4
        
        self.patch_embed1 = SpeicalPatchEmbed(
            img_size=img_size, patch_size=patch_size, stride=stride, in_chans=in_chans, embed_dim=embed_dim[0])
        ...
        self.patch_embed2 = PatchEmbed(
            img_size=img_size, patch_size=patch_size, stride=stride, in_chans=embed_dim[0], embed_dim=embed_dim[1], std=std)
        ...
        self.patch_embed3 = PatchEmbed(
            img_size=img_size, patch_size=patch_size, stride=stride, in_chans=embed_dim[1], embed_dim=embed_dim[2], std=std)
        ...
        self.patch_embed4 = PatchEmbed(
            img_size=img_size, patch_size=patch_size, stride=stride, in_chans=embed_dim[2], embed_dim=embed_dim[3], std=std)
