...
class Uniformerframe(nn.Module):
    """ Vision Transformer
    ...
    def __init__(self, cfg):
        ...
        self.frame_base = cfg.UNIFORMER.FRAME_BASE
        ...
        if representation_size:
            # if pre_logit_by_pool:
            #     self.pre_logits = nn.AvgPool3d()
            # else:
    ...
    def forward(self, x, pm=None, feat_extract=False):
        if feat_extract:
            x = x[0]
            x = self.forward_features(x)
            x = x.flatten(2).mean(-1)
