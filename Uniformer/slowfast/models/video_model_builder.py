...
from slowfast.models.utils import round_width, prod
        ...
        if self.enable_detection:
            ...
            if cfg.TEST.PROCESS:
                if len(cfg.DATA.TEST_CROP_SIZE_RECT) != 0:
                    spat_sz_list = (int(math.ceil(cfg.DATA.TEST_CROP_SIZE_RECT[0] / 32.0)),
                                    int(math.ceil(cfg.DATA.TEST_CROP_SIZE_RECT[1] / 32.0)))
                else:
                    spat_sz = int(math.ceil(cfg.DATA.TEST_CROP_SIZE / 32.0))
            ...
            self.head = head_helper.X3DHead(
                ...
                num_classes=cfg.MODEL.NUM_CLASSES,
                pool_size=[cfg.DATA.NUM_FRAMES, spat_sz_list[0], spat_sz_list[1]],
                dropout_rate=cfg.MODEL.DROPOUT_RATE,
                act_func=cfg.MODEL.HEAD_ACT,
                bn_lin5_on=cfg.X3D.BN_LIN5,
            )

    def forward(self, x, pm=None, bboxes=None):
        if pm is not None:
            pm = torch.cat(pm)
