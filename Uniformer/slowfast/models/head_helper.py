...
class X3DHead(nn.Module):
    ...
    def _construct_head(self, dim_in, dim_inner, dim_out, norm_module):
        ...
        if self.pool_size is None:
            ...
            h, w = self.pool_size[2], self.pool_size[1]
            # self.avg_pool = nn.AvgPool3d(self.pool_size, stride=1)
            # self.avg_pool_pm = nn.AvgPool3d([self.pool_size[0], self.pool_size[2], self.pool_size[1]], stride=1)
            self.avg_pool = nn.AvgPool3d([self.pool_size[0], w if h > w else h, h if h > w else w], stride=1)
            self.avg_pool_pm = nn.AvgPool3d([self.pool_size[0], h if h > w else w, w if h > w else h], stride=1)
