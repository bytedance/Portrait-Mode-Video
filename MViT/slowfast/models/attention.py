...
class MultiScaleAttention(nn.Module):
    def __init__(
        ...
        self.mode = mode
        self.hw_switch_auto=hw_switch_auto
        ...
        if self.rel_pos_spatial:
            ...
            size_h = input_size[1]
            size_w = input_size[2]
            q_size_h = size_h // stride_q[1] if len(stride_q) > 0 else size_h
            kv_size_h = size_h // stride_kv[1] if len(stride_kv) > 0 else size_h
            rel_sp_dim_h = 2 * max(q_size_h, kv_size_h) - 1

            q_size_w = size_w // stride_q[2] if len(stride_q) > 0 else size_w
            kv_size_w = size_w // stride_kv[2] if len(stride_kv) > 0 else size_w
    ...
    def forward(self, x, thw_shape):
        ...
        if self.rel_pos_spatial:
            if self.hw_switch_auto and thw_shape[1] > thw_shape[2]:
