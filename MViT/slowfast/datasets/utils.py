...
def scale_adjust_short_side_scale_jitter(min_scale, max_scale, rect_crop_size, h, w):
    if h >= w:
        # min_scale_odim = round(h / w * min_scale)
        if rect_crop_size[0] > rect_crop_size[1]:
            min_scale_new = max(min_scale, math.ceil(w / float(h) * rect_crop_size[0]))
        else:
            min_scale_new = max(min_scale, rect_crop_size[1])
    else:
        if rect_crop_size[0] > rect_crop_size[1]:
            min_scale_new = max(min_scale, rect_crop_size[0])
        else:
            min_scale_new = max(min_scale, math.ceil(h / float(w) * rect_crop_size[1]))

    max_scale_new = max_scale

    return min_scale_new, max_scale_new
...
def spatial_sampling(
    ...
    """
    if len(rect_crop_size) == 0:
        rect_crop_size = None
    # assert spatial_idx in [-1, 0, 1, 2]
    assert spatial_idx in [-2, -1, 0, 1, 2]
    if spatial_idx == -1:
        if aspect_ratio is None and scale is None:
            # if rect_crop_size is not None and auto_adjust:
            #     try:
            #         min_scale, max_scale = scale_adjust_short_side_scale_jitter(min_scale, 
            #                                                                     max_scale, 
            #                                                                     rect_crop_size, 
            #                                                                     frames.shape[2],
            #                                                                     frames.shape[3])
            ...
            if rect_crop_size is None:
                frames, _ = transform.random_crop(frames, crop_size)
            else:
                frames, _ = transform.random_crop_rect(frames, rect_crop_size)
        else:
            transform_func = (
            ...
            if rect_crop_size is None:
                ...
                frames = transform_func(
                    images=frames,
                    target_height=rect_crop_size[0],
                    target_width=rect_crop_size[1],
                    scale=scale,
                    ratio=aspect_ratio,
                    switch_hw=switch_hw,
                )
        ...
        if spatial_idx == -2:
            if rect_crop_size is not None and auto_adjust:
                raise NotImplementedError
            assert rel_center_ratio is not None
            new_h, new_w = frames.shape[-2:]
            # ratio: h, w
            offset_h = math.ceil((new_h - crop_size) * rel_center_ratio[0])
            offset_w = math.ceil((new_w - crop_size) * rel_center_ratio[1])
            if offset_h < 0: 
                offset_h = 0
            if offset_w < 0: 
                offset_w = 0
            if offset_h + crop_size > new_h: 
                offset_h = new_h - crop_size
            if offset_w + crop_size > new_w: 
                offset_w = new_w - crop_size

            frames, _ = transform.specified_crop(frames, crop_size, center_ords=[offset_w, offset_h])
        else:
            if rect_crop_size is None:
                frames, _ = transform.uniform_crop(frames, crop_size, spatial_idx)
            else:
                frames, _ = transform.uniform_crop_rect(frames, rect_crop_size, spatial_idx)
    return frames
