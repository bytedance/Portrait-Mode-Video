...
from . import decoder_av as decoder
...
class Kineticsav(torch.utils.data.Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        for _ in range(self._num_retries):
            ...
            if frames is None:
                logger.warning(f'Error: Decoding rgb frames from {self._path_to_videos[index]}')
                index = random.randint(0, len(self._path_to_videos) - 1)
                continue
            ...
            if self.cfg.DATA.USE_AUDIO and audio_frames is None:
                logger.warning(f'Error: Decoding audio frames from {self._path_to_videos[index]}')
                index = random.randint(0, len(self._path_to_videos) - 1)
                continue
