...
def random_crop_rect(images, size, boxes=None):
    """
    Perform random spatial crop on the given images and corresponding boxes.
    Args:
        images (tensor): images to perform random crop. The dimension is
            `num frames` x `channel` x `height` x `width`.
        size (list[int]): the size of height and width to crop on the image.
        boxes (ndarray or None): optional. Corresponding boxes to images.
            Dimension is `num boxes` x 4.
    Returns:
        cropped (tensor): cropped images with dimension of
    ...
    if images.shape[2] == size[0] and images.shape[3] == size[1]:
        return images
    height = images.shape[2]
    width = images.shape[3]
    y_offset = 0
    ...
    cropped = images[
        :, :, y_offset : y_offset + size[0], x_offset : x_offset + size[1]
    ]
...
def specified_crop(images, size, rel_center_ords=None, center_ords=None, boxes=None, scale_size=None):
    """
    ...
    # assert isinstance(rel_center_ords, list) and len(rel_center_ords) == 2
    ...
    #     elif spatial_idx == 2:
    #         x_offset = width - size
    if rel_center_ords is not None:
        x_offset = int(math.ceil(width * rel_center_ords[0])) - math.floor(size/2)
        y_offset = int(math.ceil(height * rel_center_ords[1])) - math.floor(size/2)
    if center_ords is not None:
        x_offset = center_ords[0]
        y_offset = center_ords[1]
...
def uniform_crop_rect(images, size, spatial_idx, boxes=None, scale_size=None):
    """
    Perform uniform spatial sampling on the images and corresponding boxes.
    Args:
        images (tensor): images to perform uniform crop. The dimension is
...
def random_resized_crop(
    ...
    i, j, h, w = _get_param_spatial_crop(scale, ratio, height, width, switch_hw=switch_hw)
