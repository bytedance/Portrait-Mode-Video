...
def train_epoch(
    ...
    for cur_iter, component in enumerate(
        ...
        if len(component) == 5:
            inputs, labels, index, time, meta = component
        elif len(component) == 4:
            inputs, labels, index, time = component
            meta = {}
        else:
            raise ValueError
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel":
                ...
                if 'pm' in meta:
                    preds = model(inputs, meta["pm"])
                else:
                    # the original models doesn't accept pm input
                    preds = model(inputs)
...
def eval_epoch(
    ...
    for cur_iter, component in enumerate(val_loader):
        if len(component) == 5:
            inputs, labels, index, time, meta = component
        elif len(component) == 4:
            inputs, labels, index, time = component
